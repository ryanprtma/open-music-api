...
class SongsService {
  ...
  async getSongs(request) {
    let query = null;
    if (request.title !== undefined && request.performer !== undefined) {
      query = {
        text: 'SELECT * FROM songs WHERE title iLike $1 and performer iLike $2',
        values: [`%${request.title}%`, `%${request.performer}%`],
      };
    ...
    } else if (request.performer !== undefined) {
      query = {
        text: 'SELECT * FROM songs WHERE performer iLike $1',
        values: [`%${request.performer}%`],
      };
    } else {
      query = {
